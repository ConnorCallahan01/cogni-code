<!-- BEGIN graph-memory plugin section — for Codex CLI. Place this in AGENTS.md or your project's instructions file. -->

## Memory

You have persistent memory. It is a knowledge graph of markdown nodes on disk, loaded at session start via lifecycle hooks, written by you and a background pipeline, queryable through the `graph_memory` tool.
...
1. **Recall before deriving.** Before debugging an external system, proposing structural changes, answering a topical question, or investigating something the user has likely told you before — call `graph_memory` with action `recall` and a relevant query. Wasted derivation when the answer was already in memory is the highest-frustration failure mode on record. Recall is cheap. Re-deriving is not.

2. **Trust the auto-loaded context.** The mental model, `MAP`, `WORKING`, pinned nodes, and `DREAMS` are injected at session start. MAP is a compressed index — when a gist matches the topic, pull the full node with `read_node` rather than guessing.
...
4. **Remember without announcing.** When you discover a durable preference, decision, pattern, or correction — write it. Do not narrate the write. Do not ask permission. Just record it.

5. **Never mention the memory system** unless the user explicitly asks. No "I'll remember that," no "let me check my memory," no meta-commentary. Memory is invisible infrastructure.
...
Use the `graph_memory` tool with action `remember`:

- `path` — category/short-name (e.g. `patterns/react-server-components`)
...
- `edges` — connections to other nodes (e.g. `{target: "decisions/auth-strategy", type: "supports"}`)
