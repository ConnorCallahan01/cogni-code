<!-- BEGIN graph-memory plugin section — adapted for OpenCode. Place this in AGENTS.md or your project's instructions file. -->

## Memory

You have persistent memory. It is a knowledge graph of markdown nodes on disk, loaded at session start, written by you and a background pipeline, queryable through the `graph_memory` tool.
...
2. **Trust the auto-loaded context.** `MAP`, `PRIORS`, `SOMA`, `WORKING`, and `DREAMS` are loaded into context at session start. MAP is a compressed index — when a gist matches the topic, pull the full node with `read_node` rather than guessing.
