...
const SCRIBE_PROMPT = fs.readFileSync(
  path.join(CONFIG.paths.prompts, "scribe.md"),
  "utf-8"
);
...
export async function fireScribe(params: {
  ...
  try {
    ...
    if (response.stop_reason !== "end_turn") {
      activityBus.log("scribe:error", `Scribe ${scribeId} response truncated (stop_reason: ${response.stop_reason}).`, {
        scribeId,
    ...
    try {
      ...
      activityBus.log("scribe:error", `Scribe ${scribeId} JSON extraction failed.`, {
        scribeId,
