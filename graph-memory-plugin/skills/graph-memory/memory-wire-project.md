...
description: Wire graph-memory awareness into the current project's CLAUDE.md. Idempotent — safe to re-run. Use when setting up memory in a new project or refreshing the memory configuration section.
---
...
1. Resolve the target `CLAUDE.md` — look in the repo root if in a git repo, else the current working directory.
