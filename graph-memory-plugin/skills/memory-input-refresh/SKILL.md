---
name: memory-input-refresh
description: Refresh Gmail and Calendar inputs on the host so the latest communication tasks can feed the morning brief. Use when the user wants to pull in fresh external data for memory context.
---
...
1. Call `graph_memory(action="status")` first so you know the graph root.
2. Run:
