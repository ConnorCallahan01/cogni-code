...
2. Run `bash` commands to inspect the host environment:
   - `command -v gws`
   - `${CLAUDE_PLUGIN_ROOT}/bin/external-inputs.sh status`
...
5. Initialize or update graph-memory external-input config with the host helper:
   - `${CLAUDE_PLUGIN_ROOT}/bin/external-inputs.sh init`
   - `${CLAUDE_PLUGIN_ROOT}/bin/external-inputs.sh enable-gmail`
   - `${CLAUDE_PLUGIN_ROOT}/bin/external-inputs.sh enable-calendar`
