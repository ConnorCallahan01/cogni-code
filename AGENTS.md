...
graph-memory-plugin/
  src/graph-memory/       # Core graph logic, runtime, inputs, pipeline
    ...
    adapters/             # Harness adapters (Codex, opencode, pi, codex)
    scripts/              # Migration and utility scripts
  src/hooks/              # Codex hooks
  agents/                 # Background agent instruction files (pipeline prompts, notion sync/merge/inbound)
  commands/               # Slash command specs (Codex)
  ...
  templates/              # Memory section templates (Codex, OpenCode, generic)
...
The memory system runs automatically via hooks (Codex) or plugin events (OpenCode):
...
- **Bootstrap** — auto-generates project docs (AGENTS.md / AGENT.md) from mental models
...
The `graph_memory` tool is available in Codex, OpenCode, and pi sessions after installation. Common actions:
...
The `graph_memory` tool provides persistent knowledge graph access across Codex, OpenCode, and pi sessions. Use it for recall, search, and remembering across sessions.
